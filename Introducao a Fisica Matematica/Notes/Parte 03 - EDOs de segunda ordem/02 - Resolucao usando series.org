...
Se \( p(x) \) e \( q(x) \) forem analíticas em \( x = x_0 \), então \( x = x_0 \) é um *ponto
...
Se \( p(x) \) *ou* \( q(x) \) não forem analíticas em \( x = x_0 \) , mas \( (x - x_0) p(x) \) e \(
(x-x_0)^2 q(x) \) forem, então
\( x = x_0 \) é dito ser um *ponto singular regular*. Caso contrário \( x = x_0 \) é um ponto singular
irregular( esse caso não é tratado nessa disciplina, então não tentei buscar entender ainda...).

+ *Exemplo*: \( x^2 y'' + 2x y' + 3y = 0 \Leftrightarrow y'' + \frac{2}{x}y ' + \frac{3}{x^2}y = 0 \)
 ...
 * *Exemplo*: \( y'' + \frac{2}{x}y' + \frac{3}{x^2}y = 0, x_0 = 0 \) 
...
*** Caso \( \lambda  \pm \in \mathbb{R} ( \lambda _+ \geq \lambda _-) \) 

**** Subcaso: \( \lambda _+ = \lambda _- \) 
...
+ Exercicio proposto: resolver para \( \nu = 0, \nu = \frac{1}{2}, \nu = 1 \) 
